...
const {
  createClient, printGraphQLError, queryAll, queryOnce,
} = require('./lib');
const {
  ArticleNode,
  BlogNode,
  ...
  try {
    ...
    await Promise.all([
      ...
      createNodes('products', PRODUCTS_QUERY, ProductNode, args, async (x) => {
        if (x.variants) await forEach(x.variants.edges, async edge => createNode(await ProductVariantNode(imageArgs)(edge.node)));

        if (x.options) await forEach(x.options, async option => createNode(await ProductOptionNode(imageArgs)(option)));
      }),
